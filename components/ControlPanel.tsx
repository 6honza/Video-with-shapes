...
import { PhysicsConfig, VisualConfig, CounterConfig, ProductionConfig, ProductionMode, MelodyType, BallShape, SoundWaveform, SpawnType, Template } from '../types';
import { Palette, Zap, Type, Play, Pause, RefreshCw, Video, Target, Plus, Clapperboard, Waves, Music, RotateCcw, Shapes, Sparkles, Ghost, Gauge, Snowflake, Volume2, Wand2, Dna, Rocket, Skull, Save, Download, Trash, FileJson, Code, HardDrive, Monitor, Layers, Bomb, Upload, Image as ImageIcon, Eye } from 'lucide-react';
...
const ControlPanel: React.FC<Props> = ({
  ...
  lastBlobUrl, onManualDownload, onExplodeAll
}) => {
  ...
  return (
    <div className="absolute left-6 top-6 bottom-6 w-[23rem] bg-zinc-950/90 backdrop-blur-3xl border border-white/10 rounded-[2.5rem] flex flex-col shadow-2xl overflow-hidden text-zinc-100 z-50">
      <div className="p-8 flex items-center justify-between bg-white/5 border-b border-white/5">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[9px] text-zinc-500 font-mono uppercase">Framework v2.3</p>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-8 space-y-10">
        {tab === 'vis' && (
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2">
            ...
            <Section icon={Ghost} label="The Arc">
              ...
              <Toggle label="Gradient Arc" active={visuals.arcGradientEnabled} onClick={() => update(visuals, setVisuals, 'arcGradientEnabled', !visuals.arcGradientEnabled)} />
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2">
            ...
            <Section icon={Type} label="Overlays">
              ...
              <div className="flex gap-2 mt-2">
                {['400', '700', '900'].map(w => (
                  <button key={w} onClick={() => update(visuals, setVisuals, 'overlayFontWeight', w)} className={`flex-1 py-2 text-[10px] rounded-lg border ${visuals.overlayFontWeight === w ? 'bg-indigo-600 border-indigo-500' : 'bg-white/5 border-white/5 text-zinc-500'}`}>
                    {w === '400' ? 'Normal' : w === '700' ? 'Bold' : 'Heavy'}
                  </button>
                ))}
              </div>
            </Section>
            
            <Section icon={RefreshCw} label="Timer Framework">
              <Toggle label="Enable Timer" active={counter.enabled} onClick={() => update(counter, setCounter, 'enabled', !counter.enabled)} />
              <Control label="Duration" v={counter.seconds} min={1} max={60} onChange={v => update(counter, setCounter, 'seconds', v)} />
        ...
        {tab === 'ai' && (
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2">
            <Section icon={Dna} label="Smart Generation">
              <p className="text-[10px] text-zinc-400 leading-relaxed mb-4">
                Describe the physics mode you want (e.g., "super fast bouncy heavy balls with blue theme"). The AI Engine will construct a configuration based on your keywords.
              </p>
              <textarea 
                value={aiPrompt}
                onChange={(e) => setAiPrompt(e.target.value)}
                placeholder="Describe your simulation..."
                className="w-full h-32 p-4 bg-white/5 border border-white/10 rounded-2xl text-xs font-mono text-white outline-none focus:border-indigo-500 transition-colors resize-none mb-4"
              />
               <button onClick={handleAiGenerate} className={`w-full py-5 rounded-2xl font-black text-xs uppercase tracking-widest transition-all bg-gradient-to-r from-indigo-600 to-purple-600 text-white hover:from-indigo-500 hover:to-purple-500 shadow-xl shadow-indigo-600/30`}>
                < Wand2 className="w-5 h-5 inline mr-3" /> Generate Mode
              </button>
            </Section>
             <Section icon={Shapes} label="Image Analysis">
               <p className="text-[10px] text-zinc-400 leading-relaxed mb-2">
                 Paste an image URL to extract physics parameters from visual style (Simulation).
               </p>
               <input type="text" placeholder="https://..." className="w-full p-3 bg-white/5 border border-white/10 rounded-xl text-[10px] text-zinc-300 mb-2"/>
               <button onClick={() => setAiPrompt("Analyzed Image: Chaos High Gravity Neon")} className="w-full py-3 bg-white/5 rounded-xl text-[9px] uppercase font-bold text-zinc-400 hover:text-white">Analyze Visuals</button>
             </Section>
          </div>
        )}
        ...
        {tab === 'prd' && (
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2">
            <Section icon={Video} label="Export Settings">
              <Control label="Force Stop After (s)" v={production.videoDuration} min={1} max={1200} onChange={v => update(production, setProduction, 'videoDuration', v)} />
              <Control label="Min Length Filter (s)" v={production.minVideoDuration} min={0} max={60} onChange={v => update(production, setProduction, 'minVideoDuration', v)} />
              ...
              <Toggle label="Cinematic Out" active={production.cinematicOut} onClick={() => update(production, setProduction, 'cinematicOut', !production.cinematicOut)} />
              {production.cinematicOut && (
                  <div className="pl-4 border-l border-white/5 space-y-4 mt-2">
                      <Control label="Zoom Speed" v={production.cinematicZoomSpeed} min={0} max={0.05} step={0.001} onChange={v => update(production, setProduction, 'cinematicZoomSpeed', v)} />
                      <Toggle label="End Dust FX" active={production.cinematicDust} onClick={() => update(production, setProduction, 'cinematicDust', !production.cinematicDust)} />
                      <Toggle label="Everything Explodes" active={production.cinematicExplosion} onClick={() => update(production, setProduction, 'cinematicExplosion', !production.cinematicExplosion)} />
                  </div>
              )}
              <div className="pt-2 border-t border-white/5 space-y-2">
                  <Toggle label="Auto Save to Disk" active={production.autoSave} onClick={() => update(production, setProduction, 'autoSave', !production.autoSave)} />
                  <Toggle label="Force 1080x1920 (HD)" active={production.forceHD} onClick={() => update(production, setProduction, 'forceHD', !production.forceHD)} />
                  <Toggle label="GPU Priority (Faster)" active={production.gpuPriority} onClick={() => update(production, setProduction, 'gpuPriority', !production.gpuPriority)} />
                  <Control label="Resolution Scale" v={production.resolutionScale} min={1} max={3} step={0.5} onChange={v => update(production, setProduction, 'resolutionScale', v)} />
              </div>
              
              <div className="pt-4 border-t border-white/5">
                 <p className="text-[9px] text-zinc-500 mb-2 uppercase font-black">Batch Processing</p>
                 <Control label="Batch Count" v={production.batchCount} min={1} max={100} onChange={v => update(production, setProduction, 'batchCount', v)} />
              </div>
            </Section>
            
            <div className="space-y-2">
                <button onClick={onStartBatch} disabled={isRecording} className={`w-full py-5 rounded-2xl font-black text-xs uppercase tracking-widest transition-all ${isRecording ? 'bg-zinc-800 text-zinc-500' : 'bg-indigo-600 text-white hover:bg-indigo-500 shadow-xl shadow-indigo-600/30'}`}>
                    <Waves className="w-5 h-5 inline mr-3" /> {isRecording ? 'Exporting...' : 'Start Render'}
                </button>
                {lastBlobUrl && (
                    <button onClick={onManualDownload} className="w-full py-3 rounded-2xl font-black text-xs uppercase tracking-widest transition-all bg-emerald-600 text-white hover:bg-emerald-500 shadow-xl shadow-emerald-600/30 animate-in fade-in slide-in-from-bottom-2">
                        <HardDrive className="w-4 h-4 inline mr-2" /> Download Last
                    </button>
                )}
            </div>
          </div>
      ...
      <div className="p-8 bg-black/40 border-t border-white/5">
        <div className="flex gap-4">
          <button onClick={() => setIsRunning(!isRunning)} className={`flex-1 py-4 rounded-xl font-black text-xs uppercase tracking-widest transition-all ${isRunning ? 'bg-zinc-800 text-zinc-400' : 'bg-white text-black'}`}>{isRunning ? <Pause className="w-5 h-5 mx-auto" /> : <Play className="w-5 h-5 mx-auto" />}</button>
          <button onClick={onReset} className="p-4 bg-white/5 rounded-xl border border-white/10 hover:bg-white/10 transition-all"><RefreshCw className="w-5 h-5 text-zinc-400" /></button>
          <button onClick={onStartBatch} disabled={isRecording} className={`flex-1 py-4 rounded-xl font-black text-xs uppercase tracking-widest transition-all bg-indigo-600 text-white`}><Video className="w-5 h-5 mx-auto" /></button>
        </div>
      </div>
    </div>
  );
...
const Color = ({ label, v, onChange }: any) => (
  <div className="flex items-center justify-between group">
    ...
    <input type="color" value={v} onChange={e => onChange(e.target.value)} className="w-8 h-8 bg-transparent border-none cursor-pointer rounded-lg overflow-hidden" />
  </div>
);
...
const Control = ({ label, v, min, max, step = 1, onChange }: any) => (
  <div className="space-y-2.5">
    <div className="flex justify-between items-center">
      <span className="text-[10px] font-black uppercase text-zinc-500 tracking-wider">{label}</span>
      <span className="text-[10px] font-mono text-indigo-400 font-black">{typeof v === 'number' ? v.toFixed(step < 0.1 ? 3 : 0) : v}</span>
    </div>
    <input type="range" min={min} max={max} step={step} value={v} onChange={e => onChange(parseFloat(e.target.value))} className="w-full h-1 bg-white/10 rounded-full appearance-none accent-indigo-500 cursor-pointer" />
  </div>
);

const Toggle = ({ label, active, onClick }: any) => (
  <button onClick={onClick} className={`w-full flex justify-between items-center p-3.5 rounded-xl transition-all border ${active ? 'bg-indigo-600/10 border-indigo-500/30' : 'bg-transparent border-white/5'}`}>
    <span className={`text-[10px] font-black uppercase tracking-tight ${active ? 'text-zinc-100' : 'text-zinc-500'}`}>{label}</span>
    <div className={`w-8 h-4 rounded-full p-1 transition-all ${active ? 'bg-indigo-600' : 'bg-zinc-800'}`}>
      <div className={`w-2 h-2 bg-white rounded-full transition-all ${active ? 'translate-x-4' : 'translate-x-0'}`} />
    </div>
  </button>
);
